...
# rescale a design: 
rescale_design = function(design, lower, upper) {
  n = ncol(design)
  for (i in 1:n) {
    design[, i] = design[, i] * (upper[i] - lower[i]) + lower[i]
  }
  design = as.data.frame(design)
  colnames(design) = paste0("x", 1:n)
  return(design)
}

# generate a lhd:
generate_lhd = function (n, lower, upper, type) {
  assert_choice(x = type, choices = c("improved", "random", 
                                      "maximin", "optimum"), null.ok = TRUE)
  lh = switch(type, improved = lhs::improvedLHS, random = lhs::randomLHS, 
              maximin = lhs::maximinLHS, optimum = lhs::optimumLHS)
  design = lh(n = n, k = length(lower))
  design = rescale_design(design = design, lower = lower, upper = upper)
  return(design)
}
...
dmvnorm_par = function(x, par) {
  dim = length(par$mean)
  x = test_dmv(x = x, ncols = dim, len = dim)
  return(mixtools::dmvnorm(y = x, mu = par$mean, sigma = par$sigma))
}
...
f_artificial_2dx2dc = function (x, c) {
  x = test_dmv(x = x, ncols = 2)
  c = test_dmv(x = c, ncols = 2)
  return(5 * x[, 1] * cos(c[, 1] - x[, 1]^2) + 
           20 * sin(x[, 2] - c[, 2]) + x[, 1] * x[, 2])
}
...
int_lim = function (values, f = 3) {
  ...
  H = ks::Hpi(x = values)
  lower = apply(values, 2, min) - f * sqrt(diag(H))
  upper = apply(values, 2, max) + f * sqrt(diag(H))
